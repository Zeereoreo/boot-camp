...
// 1) 차례대로 처음 n개의 요소를 갖는 배열을 리턴해야 합니다
_.take = function (arr, n) {
  // TODO: 여기에 코드를 작성합니다.
  if (n < 0 || n === undefined || n === 0) return [];
  if (n > arr.length) {
    return arr;
  }
  else {
...
_.drop = function (arr, n) {
  ...
  if (n < arr.length) return _.slice(arr, n);
  // start부터 end까지 자르는 것. [1,2,3]이면 0,2일 때 [1,2]
  // 마지막 인덱스.. n부터 마지막 인덱스까지.. 
};
...
_.last = function (arr, n) {
  // TODO: 여기에 코드를 작성합니다.
  if (n === 0) return [];
  if (n === undefined) return _.drop(arr, -1)
  // 마지막(start지점)부터 그 앞(start-1)까지 // slice
  if (n > arr.length) return arr;
  if (n < arr.length) return _.drop(arr, -n)

  // 1) 차례대로 마지막 n개의 요소를 갖는 배열을 리턴해야 합니다
};
 ...
 *
 * iteratee는 차례대로 데이터(element 또는 value), 접근자(index 또는 key), collection을 다룰 수 있어야 합니다. << 이걸 풀어서 쓰는 게 요구조건
...
_.each = function (collection, iteratee) {
  ...
  if (typeof collection === 'object'){
    if (Array.isArray(collection) === true){
      // 배열
      for(let i = 0; i < collection.length; i++){            // ********* for of 문은 왜 되지 않을까??
  ...
  // 4) 객체를 입력받은 경우, 객체의 속성값에 접근할 수 있어야 합니다
  // 5) 객체를 입력받은 경우, 객체의 속성값, 속성이름에 접근할 수 있어야 합니다
  // 6) 객체를 입력받은 경우, 객체의 속성값, 속성이름, 객체 자체에 접근할 수 있어야 합니다
  // 7) 객체를 입력받은 경우, `length` 속성(property)은 배열의 length와는 다르다는 것을 이해해야 합니다
};
...
_.filter = function (arr, test) {   
  ...
  return result;
      ...
      // 2) callback으로 짝수를 판별하는 함수를 입력받은 경우, 짝수만을 요소로 갖는 배열을 리턴해야 합니다
      // 3) callback으로 문자열의 길이가 6이상인지 판별하는 함수를 입력받은 경우, 길이 6 이상의 문자열만을 요소로 갖는 배열을 리턴해야 합니다
      // 4) callback의 결과가 falsy인 요소를 제외한 새로운 배열을 리턴해야 합니다
};
...
_.reject = function (arr, test) {
  ...
  // 1) _.each 함수를 사용해야 합니다
  // 2) callback으로 짝수를 판별하는 함수를 입력받은 경우, 짝수가 포함되지 않은 배열을 리턴해야 합니다
  // 3) callback으로 문자열의 길이가 6이상인지 판별하는 함수를 입력받은 경우, 길이 6 미만의 문자열만 요소로 갖는 배열을 리턴해야 합니다
};
...
_.uniq = function (arr) {
  ...
  _.each(arr, function(item){
    ...
    if(_.indexOf(arr2, item) === -1){
      // 같다면 걔만 빼기? 자르기..? -> 중복되지 않은 값을 새로운 배열로 반환
      arr2.push(item)
    }


    // 배열의 값을 모두 순회함.... [1,2,1,3]이면 1-2-1-3의 순서대로.. 요소(1,2,1,3) / 인덱스(0,1,2,3)
    // item[index] === item[index+1] 숫자값이.. 아 index.length값
    // result[index]랑 비교..
  ...
  // 1) _.each 함수를 사용해야 합니다
  // 2) 중복된 요소가 제거된 배열을 리턴해야 합니다 ★
  
  // 3) 중복 여부의 판단은 엄격한 동치 연산(strict equality, ===)을 사용해야 합니다
  // 4) 입력받은 배열을 변경하지 않고 새로운 배열을 리턴해야 합니다
};
...
_.map = function (arr, iteratee) {
  ...
  // 1) _.each 함수를 사용해야 합니다
  // 2) 전달받은 callback 함수를 배열의 모든 요소에 적용해야 합니다
  // √ 입력받은 배열을 변경하지 않고 새로운 배열을 리턴해야 합니다
  // 3) 입력받은 배열을 변경하지 않고 새로운 배열을 리턴해야 합니다
...
_.pluck = function (arr, keyOrIdx) {
  ...
  return result
  // 4) _.map 함수를 사용해야 합니다
  // 5) 각 요소에서 원하는 속성의 값만을 요소로 갖는 배열을 리턴해야 합니다
  // 6) 입력받은 배열을 변경하지 않고 새로운 배열을 리턴해야 합니다
};
...
_.reduce = function (arr, iteratee, initVal) {
  ...
  _.each(arr, function(item, index, arr){
    if(result === undefined){
      ...
      return result;
  // 3) 초기 값(accumulator)을 입력받지 않은 경우, 배열의 첫 번째 요소를 accumulator로 사용해야 합니다
  // 4) 초기 값(accumulator)을 입력받지 않은 경우, 배열의 2번째 요소부터 반복 작업을 해야 합니다
};